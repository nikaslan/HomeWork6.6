    ...
    class Program
    {
        ...
        static void ReadEntrysStream(string filepath)
        {
            ...
            using (StreamReader sr = new StreamReader(filepath))
            {
                ...
                {
                    string emptyCheck = sr.ReadLine();
                    if (emptyCheck != "")
                    {
                        string[] entry = emptyCheck.Split('#');
        ...
        static void AddEntryStream(string filepath)
        {
                ...
                if (File.Exists(filepath)) // высчитываем ID для новой записи = текущее количество строк в файле + 1
                {
                    ...
                    newEntry[0] = Convert.ToString(lineCount + 1);
                }
                #region ввод данных
                ...
                while (true)
                {
                    ...
                    if (key == ConsoleKey.Enter)
                    {
                        ...
                        sw.WriteLine(toAppend); //добавление новой записи в файл
                        }
